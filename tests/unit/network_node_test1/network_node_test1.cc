...
int main(int argc, char *argv[])
{
   ...
   vector<SInt32> num_output_endpoints_list(num_channels, 1);
   ...
   vector<SInt32> input_buffer_size_list(num_channels, 1);
   vector<SInt32> downstream_buffer_size_list(num_channels, 2);
   ...
   // Link Power Model
   vector<LinkPowerModel*> link_power_model_list(num_channels, (LinkPowerModel*) NULL);
   ...
   for (SInt32 i = 0; i < num_channels; i++)
   {
      ...
      NetworkNode* network_node =
         new NetworkNode(
               ...
               vector<vector<Router::Id> >(1, vector<Router::Id>(1, Router::Id(0, num_channels)) ),
               vector<vector<Router::Id> >(1, vector<Router::Id>(1, Router::Id(0, num_channels)) ),
               USER_2);
...
list<NetPacket*> createNetPacketList()
{
   ...
   // Head Flit - 0
   output_endpoints << 0 << 0 << 3 << 0 << 4 << 0;
   addFlit(net_packet_list, Flit::HEAD, 5, 1, Router::Id(0,1), Router::Id(0,5), 3, &output_endpoints);

   // Head Flit - 1
   output_endpoints << 0 << 0;
   addFlit(net_packet_list, Flit::HEAD, 6, 1, Router::Id(0,2), Router::Id(0,5), 1, &output_endpoints);

   // Tail Flit - 1
   addFlit(net_packet_list, Flit::TAIL, 7, 1, Router::Id(0,2), Router::Id(0,5));
   
   // Body Flit - 0
   addFlit(net_packet_list, Flit::BODY, 7, 1, Router::Id(0,1), Router::Id(0,5));

   // Tail Flit - 0
   addFlit(net_packet_list, Flit::TAIL, 10, 1, Router::Id(0,1), Router::Id(0,5));

   // Credit Msg
   addCreditMsg(net_packet_list, 20, 1, Router::Id(0,0), Router::Id(0,5));
   addCreditMsg(net_packet_list, 23, 1, Router::Id(0,0), Router::Id(0,5));
   addCreditMsg(net_packet_list, 25, 1, Router::Id(0,3), Router::Id(0,5));
   addCreditMsg(net_packet_list, 30, 1, Router::Id(0,4), Router::Id(0,5));
   addCreditMsg(net_packet_list, 45, 1, Router::Id(0,0), Router::Id(0,5));
