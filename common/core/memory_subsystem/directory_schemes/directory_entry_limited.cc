...
bool
DirectoryEntryLimited::hasSharer(core_id_t sharer_id)
{
   for (SInt32 i = 0; i < _max_hw_sharers; i++)
   {
...
bool
DirectoryEntryLimited::addSharer(core_id_t sharer_id)
{
   for (SInt32 i = 0; i < _max_hw_sharers; i++)
   {
...
void
DirectoryEntryLimited::removeSharer(core_id_t sharer_id)
{
   for (SInt32 i = 0; i < _max_hw_sharers; i++)
   {
...
bool
DirectoryEntryLimited::getSharersList(vector<core_id_t>& sharers_list)
{
   for (SInt32 i = 0; i < _max_hw_sharers; i++)
   {
      if (_sharers[i] != INVALID_SHARER)
         sharers_list.push_back((core_id_t) _sharers[i]);
   }
